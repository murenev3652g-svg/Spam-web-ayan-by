...
class FF_CLient():
    ...
    def Guest_GeneRaTe(self, uid, password, attempt=1):
        if attempt > 3:
            print(f"{R}❌ Max retries for guest token {self.id}{RS}")
            return None
        ...
        try:
            ...
            if 'access_token' in self.response and 'open_id' in self.response:
                ...
                print(f"{R}❌ Guest Auth Failed {self.id}{RS}")
                return None
        except Exception as e: 
            print(f"{R}❌ Login error {self.id}: {e}{RS}")
            add_console_log(f"❌ Login error {self.id}: {e}", "error")
            time.sleep(5)
            return self.Guest_GeneRaTe(uid, password, attempt + 1)
    ...
    def ToKen_GeneRaTe(self, Access_ToKen, Access_Uid, attempt=1):
        if attempt > 3:
            print(f"{R}❌ Max retries reached for {self.id}. Account might be banned or blocked.{RS}")
            return None
        ...
        if self.ResPonse.status_code == 200 and len(self.ResPonse.content) > 10:
            try:
                self.BesTo_data = json.loads(DeCode_PackEt(self.ResPonse.content.hex()))
                
                # 🔥 FIX: Safely check for JWT Token field instead of hardcoding '8'
                if '8' in self.BesTo_data and 'data' in self.BesTo_data['8']:
                    self.JwT_ToKen = self.BesTo_data['8']['data']
                elif '7' in self.BesTo_data and 'data' in self.BesTo_data['7']:
                    self.JwT_ToKen = self.BesTo_data['7']['data']
                else:
                    err_code = self.BesTo_data.get('1', {}).get('data', 'Unknown')
                    print(f"{R}❌ Login Rejected for {self.id} | Code: {err_code} (Account might be banned/blocked){RS}")
                    return None
                ...
                print(f"{R}❌ Parsing error for {self.id}: {e}{RS}")
                time.sleep(3)
                return self.ToKen_GeneRaTe(Access_ToKen, Access_Uid, attempt + 1)
        else:
            print(f"{R}❌ HTTP Error {self.ResPonse.status_code} for {self.id}{RS}")
            time.sleep(3)
            return self.ToKen_GeneRaTe(Access_ToKen, Access_Uid, attempt + 1)
    ...
    def Get_FiNal_ToKen_0115(self):
        try:
            ...
            # 🔥 FIX: Prevent tuple unpacking error if login fails
            if not result or len(result) != 8:
                print(f"{Y}⚠️ Skipping account {self.id} due to authentication failure.{RS}")
                return None
...
def main():
    ...
    app.run(host='0.0.0.2', port=port, debug=False, threaded=True)
